    ...
    <div x-data="{ openTask: null, openSubmit: null, confirmSubmit: false, openRejectReason: null, alertOpen: true }" class="min-h-screen bg-slate-50 p-6" dir="rtl">
        <header class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
            <div>
                ...
                <p class="text-slate-500 mt-1">أهلاً بك يا <?php echo e(auth()->user()->name); ?> <?php if(auth()->user()->department): ?>
                        <span class="text-indigo-500 font-medium">(<?php echo e(auth()->user()->department->name); ?>)</span>
                    <?php endif; ?> 👋</p>
            </div>
        ...
        <?php if(session('success')): ?>
            <div x-show="alertOpen" x-init="setTimeout(() => alertOpen = false, 4000)" x-transition
                ...
                <span class="flex items-center gap-2"><svg class="w-5 h-5" fill="none" stroke="currentColor"
                    ...
                    </svg> <?php echo e(session('success')); ?></span>
        ...
        <?php if($errors->any()): ?>
            <div x-show="alertOpen" x-init="setTimeout(() => alertOpen = false, 4000)" x-transition
                ...
                <span class="flex items-center gap-2"><svg class="w-5 h-5" fill="none" stroke="currentColor"
                    ...
                    </svg> <?php echo e($errors->first()); ?></span>
        ...
        <?php if($pendingTasksCount > 0): ?>
            <div x-data="{ show: true }" x-show="show" x-transition
                ...
                <span class="flex items-center gap-2"><svg class="w-5 h-5 animate-pulse" fill="none"
                    ...
                    </svg> تنبيه: لديك (<?php echo e($pendingTasksCount); ?>) مهام واردة قيد الانتظار، تأكد من إنجازها وتسليمها
                    للإدارة.</span>
        ...
        <div class="grid md:grid-cols-2 gap-6">
            <section class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                ...
                <?php if($assignedTasks->count() > 0): ?>
                    <div class="space-y-4">
                        <?php $__currentLoopData = $assignedTasks; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $task): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="p-4 bg-slate-50 rounded-xl border border-slate-100 hover:shadow-md transition cursor-pointer group"
                                @click="openTask = <?php echo e(\Illuminate\Support\Js::from($task)); ?>">
                                <div class="flex justify-between items-start mb-2">
                                    <div class="flex items-center gap-2">
                                        <span
                                            class="text-xs font-medium bg-indigo-100 text-indigo-700 px-2 py-1 rounded-full">المهمة
                                            رقم <?php echo e($assignedTasks->count() - $index); ?></span>
                                    ...
                                    <span class="text-xs text-slate-400"><?php echo e($task->task_date ? $task->task_date : $task->created_at->format('Y-m-d')); ?></span>
                                </div>
                                <p
            ...
            <section class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                ...
                <?php if($submittedTasks->count() > 0): ?>
                    <div class="space-y-3">
                        <?php $__currentLoopData = $submittedTasks; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sub): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <?php
                            ...
                            ?>
                            <div class="p-4 <?php echo e($colors['bg']); ?> border <?php echo e($colors['border']); ?> rounded-xl transition hover:shadow-sm flex justify-between items-center"
                                <?php if($sub->status === 'rejected'): ?> @click="openRejectReason = <?php echo e(\Illuminate\Support\Js::from($sub->admin_feedback)); ?>" role="button" tabindex="0" <?php endif; ?>>
                                <div>
                                    <p class="font-medium text-slate-800"><?php echo e(Str::limit($sub->description, 60)); ?></p>
                                    <span
                                        class="text-xs text-slate-500"><?php echo e($sub->submission_date ? $sub->submission_date : $sub->created_at->format('Y-m-d')); ?></span>
                                </div>
                                <span
                                    class="px-3 py-1 text-xs font-semibold rounded-full <?php echo e($colors['text']); ?> bg-white/50"><?php echo e($colors['status']); ?></span>
                            </div>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </div>
                <?php else: ?>
            ...
            <div @click.outside="openTask = null"
                ...
                <div class="flex justify-end gap-3">
                    <template x-if="openTask?.file_path">
                        <a :href="'<?php echo e(route('download')); ?>?path=' + openTask.file_path" target="_blank"
        ...
        <div x-show="openSubmit !== null" x-transition style="display: none;"
            ...
            <div @click.outside="openSubmit = null" class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
                ...
                <form id="submitTaskForm" method="POST" action="<?php echo e(route('employee.submit_task')); ?>"
                    ...
                    <div>
                        ...
                        <input type="date" name="submission_date" required value="<?php echo e(date('Y-m-d')); ?>" class="w-full px-4 py-3 rounded-xl border border-slate-300 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/20 outline-none transition">
                    </div>
                    <div x-data="{ fileName: '' }">
